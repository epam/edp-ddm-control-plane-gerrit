        ...
        script.openshift.withCluster() {
            script.openshift.withProject() {
                script.dir("${context.workDir}") {
                    ...
                    def openshiftClusterMasterMachineJSON = script.readJSON text: script.sh(script: """ oc get machine -n openshift-machine-api -l machine.openshift.io/cluster-api-machine-role=master --no-headers -o 'jsonpath={.items[*]}' """, returnStdout: true).trim()

                    script.env.CLUSTER_NAME = openshiftClusterMasterMachineJSON.metadata.labels."machine.openshift.io/cluster-api-cluster"
                    script.env.cloudProvider = openshiftClusterMasterMachineJSON.spec.providerSpec.kind.replaceAll('MachineProviderConfig','')

                    openshiftClusterMasterMachineJSON.metadata.labels.eachWithIndex { label, index ->
                        def registryMachineSet = script.readYaml file: "placeholders-templates/${script.env.cloudProvider}/registry-machine-set.yaml"
                        registryMachineSet.replaceAll('{{ .Values.node_zone }}', label."machine.openshift.io/zone").replaceAll('{{ .Values.node_region }}', label."machine.openshift.io/region")
                        script.sh "rm -rf deploy-templates/registry-nodes/templates/registry-machine-set*"
                        script.writeYaml file: "deploy-templates/registry-nodes/templates/registry-machine-set-" + label."machine.openshift.io/region" + '-' + label."machine.openshift.io/zone" + '.yaml', data: registryMachineSet
                    }
                    ...
                    script.sh (""" oc annotate namespace ${script.env.NAMESPACE} 'scheduler.alpha.kubernetes.io/defaultTolerations'='[{"operator": "Equal", "effect": "NoSchedule", "key": "node/${script.env.NAMESPACE}"}, "value": "true"]' """)
